...
int main(int argc,char** argv){
    ...
    double train_dat[TRAIN_DAT][2];
    double l, r, targets[TRAIN_DAT][1];
    for(size_t i=0;i!=TRAIN_DAT;++i){
        l = RandFloat();    r=RandFloat();
        train_dat[i][0] = l;
        train_dat[i][1] = r;
        targets[i][0] = l>r? l:r;
    }    
    cout << targets[0][0] << endl;
    ...
    for(size_t i=0;i!=max_iteration;++i){
        for(size_t k=0;k!=TRAIN_DAT;++k){
            current_err=train(layers,0.1,0.2,train_dat[k],targets[k]);
        }
        if(i%256 == 0)
